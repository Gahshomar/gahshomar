...
from .gs_calendar import PersianCalendar, GeorgianCalendar, date_to_georgian,\
    add_months, add_years
from .gs_months_widget import PersianMonthsWidget, GeorgianMonthsWidget
...
class CalendarWidget(Gtk.Box):
    ...
    def year_arrow_pressed(self, year, icon_pos, event):
...
class PersianCalendarWidget(CalendarWidget, PersianCalendar):
    ...
    def month_arrow_pressed(self, *args):
        l_r = args[-1]
        date = khayyam.JalaliDate.from_date(self.parent.date)
        if l_r == 'r':
            date = add_months(date, -1)
        else:
            date = add_months(date, 1)
        self.parent.handler.update_everything(date_to_georgian(date))
